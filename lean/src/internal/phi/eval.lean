...
lemma Φ_eval_xgrd_some { gdt_grd: XGrd } { env env': Env } (h: xgrd_eval gdt_grd env = some env') (ty: Φ):
    (Φ.xgrd_in gdt_grd ty).eval env = ty.eval env' :=
by simp [Φ.eval, h]

lemma Φ_eval_xgrd_none { gdt_grd: XGrd } { env: Env } (h: xgrd_eval gdt_grd env = none) (ty: Φ):
    (Φ.xgrd_in gdt_grd ty).eval env = ff :=
by simp [Φ.eval, h]
